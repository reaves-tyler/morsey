...
| **Stream Decoding** (`/decode`) | A live over-the-air CW terminal: feed it the rig's audio (aux / line-in, USB sound card, or a microphone at the speaker) and it prints what it hears while passing the audio through to your headphones, levelled so transmit sidetone and receive audio sit at the same volume. Spectrum display with click-to-tune passband, adaptive threshold with floor/threshold/peak meter, speed tracking, prosigns as chips, WAV recorder. Synthesized test clips (clean → hand-sent → noisy → QSB → QRM → weak) ship as WAVs under `public/samples/cw/` for the file source and drive the decoder test suite |
