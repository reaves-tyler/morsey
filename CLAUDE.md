...
| `app/composables/useKeyer.ts` | Sending-side input → dits/dahs → text. Straight key (adaptive dit-length classification, calibrates to the operator per press) and iambic paddle (machine-timed elements). Timing uses `settings.sendWpm`, NOT the receive speed. Letter/word commit gaps are stretched (4u/8u, 450 ms/1.2 s floors). Web Serial supports two flavors on one port: CTS/DSR control-line polling (passive paddles) and the `DIT_DOWN`/`DIT_UP`/`DAH_DOWN`/`DAH_UP` line protocol from the Pico bridge. Backspace clears. |
| `hardware/pico-bridge/` | MicroPython `main.py` for a Raspberry Pi Pico H paddle→USB bridge (GP14 = dit, GP15 = dah, pull-ups, 5 ms stable-state debounce) plus wiring/flash docs. Deliberately dumb: emits only debounced state lines; all timing/decoding stays in the browser. |
